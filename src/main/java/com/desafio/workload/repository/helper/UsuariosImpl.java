...
import com.desafio.workload.model.UsuarioGrupo;
...
public class UsuariosImpl implements UsuariosQueries {
	...
	public Page<Usuario> filtrar(UsuarioFilter filtro, Pageable pageable, int identificador) {
		...
		adicionarFiltro(filtro, criteria, identificador);
		
		List<Usuario> filtrados = criteria.list();
		filtrados.forEach(u -> Hibernate.initialize(u.getGrupos()));
 		
		return new PageImpl<>(filtrados, pageable, total(filtro, identificador));
	}
	...
	private Long total(UsuarioFilter filtro, int identificador) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Usuario.class);
		adicionarFiltro(filtro, criteria, identificador);
	...
	private void adicionarFiltro(UsuarioFilter filtro, Criteria criteria, int identificador) {
		if (filtro != null) {
			...
			if(identificador == 0) {
				...
				criteria.add(Restrictions.isNotNull("professor"));
				
				criteria.createAlias("professor", "p");
				
				if(!StringUtils.isEmpty(filtro.getSiape())) {
					criteria.add(Restrictions.eq("p.siape", filtro.getSiape()));
				}
			}
